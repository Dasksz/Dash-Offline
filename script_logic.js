        ...
        if (embeddedData.isColumnar) {
            ...
            allSalesData = embeddedData.detailed;
            allHistoryData = embeddedData.history;
        ...
        for(let i=0; i<allClientsData.length; i++) {
            ...
            clientMapForKPIs.set(String(c['Código']), c);
        }
        ...
        function initializeOptimizedDataStructures() {
            ...
            // Combined loop for seller details map
            const combinedData = [...allSalesData, ...allHistoryData];
            for (let i = 0; i < combinedData.length; i++) {
                const s = combinedData[i];
                if (s.CODUSUR && (!sellerDetailsMap.has(s.CODUSUR) || s.NOME)) {
            ...
            for (let i = 0; i < allClientsData.length; i++) {
                ...
                // Note: Modifying 'client' object here DOES NOT persist to Columnar Array automatically.
                // We are using the hydrated object for Maps.
                // Since 'allClientsData' is read-only from source essentially, we rely on local Maps for modified state
                // OR we accept that we can't modify in place easily.
                // However, the existing code modifies `client['Código']` string conversion.
                // ColumnarDataset.get() returns a NEW object every time.
                // So modification here is useless if we access allClientsData.get(i) later.

                // FIX: For Clients, it's safer to keep them as hydrated objects in 'clientMapForKPIs'
                // and use that map for lookups where modifications matter.
                // But filter functions iterate 'allClientsData'.
                // If we need persistent modifications (like Americanas RCA override), we need a side-store.
                // The Americanas logic modifies `client.rca1`.

                // Workaround: We'll use a `clientOverrides` map or similar,
                // OR since Clients dataset is small (thousands), we can fully hydrate it into an Array
                // and avoid Columnar complexity for Clients if it causes mutability issues.
                // Let's assume for now we just use the calculated values in the Maps.

                client.cidade = client.cidade || 'N/A';

                if ((client.razaoSocial || '').toUpperCase().includes('AMERICANAS')) {
                    client.rca1 = '1001';
                    client.rcas = ['1001'];
            ...
            const processDatasetForIndices = (data, indexSet, dataMap, isHistory) => {
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    const supervisor = sale.SUPERV;
                    const rca = sale.NOME;
                    const pasta = sale.OBSERVACAOFOR || 'N/A';
                    const supplier = sale.CODFOR;
                    const client = sale.CODCLI;
                    const position = sale.POSICAO || 'N/A';
                    const rede = clientRamoMap.get(client) || 'N/A';
                    const tipoVenda = sale.TIPOVENDA;
                    ...
                    if (product) {
                        ...
                        targetMap.get(product).push(sale);
                    }
                }
            };
        ...
        let currentGoalsSupplier = 'ELMA_ALL';
        ...
        let globalSellerGoals = {};
        let globalClientGoals = new Map(); // Map<CodCli, Map<Key, {fat: 0, vol: 0}>>
        let quarterMonths = [];
                        ...
                        if (key && globalGoalsMetrics[key]) {
                            ...
                            if (sale.TIPOVENDA === '1' || sale.TIPOVENDA === '9') {
                                ...
                                if (isPrevMonth) {
                                    ...
                                    g.fat += sale.VLVENDA;
                                    g.vol += (sale.TOTPESOLIQ / 1000); // Tons
                                }
                            }
        ...
        function exportGoalsSvXLSX() {
            ...
            svColumns.forEach(col => {
                ...
                if (col.type === 'standard') {
                    ...
                    row2.push(createCell("MÉDIA TRIM.", style), createCell("META TON", style), createCell("", style));
                ...
                sellers.forEach(seller => {
                    ...
                    svColumns.forEach(col => {
                        ...
                        if (col.type === 'standard') {
                            ...
                            rowData.push({ t: 'n', v: d.metaPos, f: fPos, s: { ...aggCellStyle, numFmt: fmtInt }, z: fmtInt });
                ...
                if (clientHistoryIds) {
                    clientHistoryIds.forEach(id => {
                        ...
                        if (key && metricsMap[key]) {
                            ...
                            // 1. Revenue/Volume metrics (Types 1 & 9) - Global Sums
                            if (sale.TIPOVENDA === '1' || sale.TIPOVENDA === '9') {
                                metricsMap[key].fat += sale.VLVENDA;
                                metricsMap[key].vol += sale.TOTPESOLIQ;

                                if (isPrevMonth) {
                                    metricsMap[key].prevFat += sale.VLVENDA;
                                    metricsMap[key].prevVol += sale.TOTPESOLIQ;
                                }
                            }
        ...
        function calculateDistributedGoals(filteredClients, currentGoalsSupplier, currentGoalsBrand, goalFat, goalVol) {
            ...
            filteredClients.forEach(client => {
                ...
                const metaPos = sumFat > 1 ? 1 : 0; // Positivado se venda > 1 (threshold padrão)
        ...
        function calculateGoalMixVariation(input) {
            ...
            if (diff > 0) {
                variationEl.textContent = `+${pct.toFixed(1)}%`;
                variationEl.className = 'text-xs font-bold text-right h-4 text-green-400';
            } else if (diff < 0) {
                variationEl.textContent = `${pct.toFixed(1)}%`;
                variationEl.className = 'text-xs font-bold text-right h-4 text-red-400';
            } else {
                variationEl.textContent = '';
            }
        }
...
function getFilterDescription() {
        ...
        function distributePositivacao() {
        ...
        function updateGoalsView() {
            ...
            // But 'calculateDistributedGoals' calculates everything in one go.
            // Let's rely on it for now but wrap it in async chunked loop if possible?
            // Actually 'calculateDistributedGoals' is the heavy function.
            // We should use 'runAsyncChunked' to process 'filteredClients'.

            // Pre-calculate Global Totals if missing (Sync because it loops all active clients once, can be heavy but needed for share)
            // Ideally this should also be cached or async. For now, assuming it's fast enough or cached.
            if (!globalGoalsTotalsCache[cacheKey]) {
                 // Trigger calculation by calling with empty list? No.
                 // We need to run the global calc logic.
                 // Let's just run calculateDistributedGoals for the first chunk to prime the cache?
                 // Or just let it run synchronously once.
            ...
            const shouldIncludeSale = (sale, supplier, brand) => {
                const codFor = String(sale.CODFOR);
                if (supplier === 'ELMA_ALL') { if (!['707', '708', '752'].includes(codFor)) return false; }
            ...
            runAsyncChunked(filteredClients, (client) => {
                ...
                const avgFat = cSumFat / QUARTERLY_DIVISOR;
                const avgVol = (cSumVol / 1000) / QUARTERLY_DIVISOR;
                let activeMonthsCount = 0;
                monthlyActivity.forEach(val => { if(val > 1) activeMonthsCount++; });
                ...
                if (currentGoalsSupplier === 'ELMA_ALL' || currentGoalsSupplier === 'FOODS_ALL') {
                    if (globalClientGoals.has(codCli)) {
                        const cGoals = globalClientGoals.get(codCli);
                        const keysToSum = currentGoalsSupplier === 'ELMA_ALL' ? ['707', '708', '752'] : ['1119_TODDYNHO', '1119_TODDY', '1119_QUAKER_KEROCOCO'];
                ...
                const metric = {
                    ...
                    avgFat, avgVol, prevFat: cPrevFat, prevVol: cPrevVol / 1000,
                ...
                totalAvgVol += avgVol; totalPrevVol += (cPrevVol / 1000);
                ...
                const isAggregatedTab = currentGoalsSupplier === 'ELMA_ALL' || currentGoalsSupplier === 'FOODS_ALL';

                if (fatInput) {
                    if (document.activeElement !== fatInput) fatInput.value = sumFat.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
                    fatInput.readOnly = false; fatInput.classList.remove('opacity-50', 'cursor-not-allowed');
                }
                if (volInput) {
                    if (document.activeElement !== volInput) volInput.value = sumVol.toLocaleString('pt-BR', { minimumFractionDigits: 3, maximumFractionDigits: 3 });
                ...
                const refAvgVol = document.getElementById('ref-avg-vol'); if(refAvgVol) refAvgVol.textContent = totalAvgVol.toLocaleString('pt-BR', { minimumFractionDigits: 3 });
                const refPrevVol = document.getElementById('ref-prev-vol'); if(refPrevVol) refPrevVol.textContent = totalPrevVol.toLocaleString('pt-BR', { minimumFractionDigits: 3 });
                ...
                // Calculate Total Meta Pos (Aggregating Effective Goals per Seller)
                let totalMetaPos = 0;

                // 1. Group natural counts by seller from the current filtered metrics
                const sellerNaturalCounts = new Map();
                clientMetrics.forEach(c => {
                    // Use seller name as key (consistent with globalSellerGoals)
                    const sName = c.seller;
                    sellerNaturalCounts.set(sName, (sellerNaturalCounts.get(sName) || 0) + c.metaPos);
                });

                // 2. Sum effective goals
                const currentKey = currentGoalsSupplier + (currentGoalsBrand ? '_' + currentGoalsBrand : '');

                sellerNaturalCounts.forEach((naturalCount, sellerName) => {
                    let effective = naturalCount;
                    if (globalSellerGoals[sellerName] && globalSellerGoals[sellerName][currentKey]) {
                        effective = globalSellerGoals[sellerName][currentKey].pos;
                    }
                    totalMetaPos += effective;
                });

                if (goalMixInput) {
                    goalMixInput.value = totalMetaPos.toLocaleString('pt-BR');
                    goalMixInput.dataset.originalValue = totalMetaPos;

                    let btnDiv = document.getElementById('goal-mix-btn-container');
                    if (!btnDiv) {
                        btnDiv = document.createElement('div');
                        btnDiv.id = 'goal-mix-btn-container';
                        btnDiv.className = 'mt-1 flex justify-end';
                        btnDiv.innerHTML = '<button id="btn-distribute-mix" class="bg-purple-600 hover:bg-purple-500 text-white px-3 py-1 rounded text-[10px] uppercase font-bold transition-colors">Definir</button>';
                        goalMixInput.parentNode.appendChild(btnDiv);
                        document.getElementById('btn-distribute-mix').addEventListener('click', () => {
                             const val = document.getElementById('goal-global-mix').value;
                             showConfirmationModal(`Deseja definir a Meta de Positivação para ${val}?`, distributePositivacao);
                        });
                    }

                    if (selectedGoalsGvSellers.length === 1) {
                        goalMixInput.readOnly = false;
                        goalMixInput.classList.remove('opacity-50', 'cursor-not-allowed');
                        btnDiv.classList.remove('hidden');
                    } else {
                        goalMixInput.readOnly = true;
                        goalMixInput.classList.add('opacity-50', 'cursor-not-allowed');
                ...
                if (clientMetrics.length === 0) {
                    ...
                    const rows = pageData.map(item => {
                        ...
                        return `<tr class="hover:bg-slate-800 group transition-colors border-b border-slate-800"><td class="px-2 py-2 text-center border-r border-slate-800 bg-[#151c36] text-xs text-slate-300">${item.cod}</td><td class="px-2 py-2 text-left border-r border-slate-800 bg-[#151c36] text-xs font-bold text-white truncate max-w-[200px]" title="${item.name}">${item.name.substring(0, 30)}</td><td class="px-2 py-2 text-left border-r border-slate-800 bg-[#151c36] text-[10px] text-slate-400 uppercase">${getFirstName(item.seller)}</td>${monthCells}<td class="px-2 py-2 text-right text-slate-300 font-medium bg-blue-900/10 border-r border-slate-800/50 text-xs">${item.avgFat.toLocaleString('pt-BR', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</td><td class="px-2 py-2 text-center text-blue-400 text-xs bg-blue-900/10 border-r border-slate-800/50">${(item.shareFat * 100).toFixed(2)}%</td><td class="px-2 py-2 text-right font-bold text-blue-200 bg-blue-900/20 border-r border-slate-800 text-xs">${item.metaFat.toLocaleString('pt-BR', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</td><td class="px-2 py-2 text-right text-slate-300 font-medium bg-orange-900/10 border-r border-slate-800/50 text-xs">${item.avgVol.toLocaleString('pt-BR', {minimumFractionDigits: 3, maximumFractionDigits: 3})}</td><td class="px-2 py-2 text-center text-orange-400 text-xs bg-orange-900/10 border-r border-slate-800/50">${(item.shareVol * 100).toFixed(2)}%</td><td class="px-2 py-2 text-right font-bold text-orange-200 bg-orange-900/20 border-r border-slate-800 text-xs">${item.metaVol.toLocaleString('pt-BR', {minimumFractionDigits: 3, maximumFractionDigits: 3})}</td><td class="px-2 py-2 text-center font-bold text-purple-300 bg-purple-900/10 text-xs">${item.metaPos}</td></tr>`;
                    }).join('');
        ...
        function getCoverageFilteredData(options = {}) {
            ...
            if (filial !== 'ambas' || supervisorsSet.size > 0 || sellersNameSet.size > 0 || city) {
                ...
                clients = allClientsData.filter(c => {
                    ...
                    if (rcasOfSupervisor.size > 0 && !c.rcas.some(rca => rcasOfSupervisor.has(rca))) return false;
                    if (rcasOfSellers.size > 0 && !c.rcas.some(rca => rcasOfSellers.has(rca))) return false;
        ...
        function updateSellerFilter(supervisors, dropdown, filterText, selectedArray, dataSource, skipRender = false) {
            ...
            if (supervisors && supervisors.length > 0) {
                ...
                sellersToShow = [...new Set(dataSource.filter(s => supSet.has(s.SUPERV)).map(s => s.NOME))].sort();
            } else {
        ...
        function renderKpiCards(kpis) {
            ...
            container.innerHTML = kpis.map(kpi => {
                ...
                return `<div class="bg-[#1d2347] p-4 rounded-lg text-center border border-slate-700 transition transform hover:-translate-y-1 duration-200">
        ...
        function calculateUnifiedMetrics(currentSales, historySales) {
            ...
            // --- DEBUG DIAGNOSTICS FOR MIX PEPSICO ---
            const debugMixList = [];
            currentClientProductMap.forEach((prods, codcli) => {
                let pepsicoCount = 0;
                prods.forEach(pData => {
                    if (pData.val > 1 && pepsicoCodfors.has(pData.codfor)) {
                        pepsicoCount++;
                    }
                });
                if (pepsicoCount > 0) {
                    debugMixList.push({ cod: codcli, mix: pepsicoCount });
                }
            });
            ...
            console.log(`Média Calculada: ${metrics.current.mixPepsico}`);
            console.log("Top 10 Clientes com MENOR Mix (Puxam a média para baixo):");
            console.table(debugMixList.slice(0, 10));
            console.log("Top 10 Clientes com MAIOR Mix:");
            console.table(debugMixList.slice(-10));
            console.groupEnd();
            // ----------------------------------------
        ...
        function updateComparisonView() {
            ...
            runAsyncChunked(currentSales, (s) => {
                ...
                runAsyncChunked(historySales, (s) => {
                    ...
                    const sortedMonths = Array.from(historyMonths.keys()).sort().slice(-3);
                    ...
                    if (dailyBreakdownDatasets.some(ds => ds.data.some(d => d > 0))) {
                        createChart('dailyWeeklyComparisonChart', 'bar', weekLabelsForDailyChart, dailyBreakdownDatasets, { plugins: { legend: { display: true, position: 'top' }, tooltip: { mode: 'point', intersect: true }, datalabels: { display: false } }, scales: { x: { stacked: false }, y: { stacked: false, ticks: { callback: (v) => (v / 1000).toFixed(0) + 'k' } } } });
                    } else {
    ...
                });
        ...
        function getInnovationsMonthFilteredData(options = {}) {
            ...
            if (excludeFilter !== 'supervisor' && selectedInnovationsSupervisors.length > 0) {
                ...
                clients = clients.filter(c => c.rcas.some(rca => rcasSet.has(rca)));
            }
        ...
        function updateInnovationsMonthView() {
            ...
            if (!globalInnovationCategories && innovationsMonthData && innovationsMonthData.length > 0) {
                ...
                innovationsMonthData.forEach(item => {
                    const categoryName = item.Inovacoes;
                    ...
                    globalInnovationCategories[categoryName].products.push({ ...item, Codigo: productCode });
        ...
        function setupEventListeners() {
            ...
            comparisonTipoVendaFilterDropdown.addEventListener('change', (e) => {
                if (e.target.type === 'checkbox') {
                    ...
                    selectedComparisonTiposVenda = updateTipoVendaFilter(comparisonTipoVendaFilterDropdown, comparisonTipoVendaFilterText, selectedComparisonTiposVenda, allSalesData);
                    handleComparisonFilterChange();
                }
            });
            ...
            stockTipoVendaFilterDropdown.addEventListener('change', (e) => {
                if (e.target.type === 'checkbox') {
                    ...
                    selectedStockTiposVenda = updateTipoVendaFilter(stockTipoVendaFilterDropdown, stockTipoVendaFilterText, selectedStockTiposVenda, allSalesData);
            ...
            const toggleGoalsView = (view) => {
                // Reset Buttons
                [btnElmaChips, btnFoods, btnSummary].forEach(btn => {
                    if (btn) {
                ...
                if (view === 'summary') {
                    ...
                    // Actually, `firstTab.click()` triggers the event listener above which handles this!
                    // So we are covered by the `click()` call.
                }
            };

            if (btnElmaChips && btnFoods && btnSummary) {
